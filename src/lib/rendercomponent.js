...
export const render = {
  renderStation: (data) => {
    ...
    if(data.Metros.length > 0){
      ...
      data.Metros.forEach(metros => {
        el.metro.innerHTML +=
        `<li class="stations-info">${metros.LineNumber} mot ${metros.Destination} ${metros.DisplayTime}</li>`;
      });
    } else {
    ...
    if(data.Trams.length > 0){
      ...
      data.Trams.forEach(trams => {
        el.trams.innerHTML +=
        `<li class="stations-info">${trams.LineNumber} mot ${trams.Destination} ${trams.DisplayTime}</li>`;
      });
    } else {
    ...
    if(data.Buses.length > 0){
      ...
      data.Buses.forEach(bus => {
        el.bus.innerHTML +=
        `<li class="stations-info">${bus.LineNumber} mot ${bus.Destination} ${bus.DisplayTime}</li>`;
      });
    } else {
  ...
  renderWeather: (name, description, temp) => {
